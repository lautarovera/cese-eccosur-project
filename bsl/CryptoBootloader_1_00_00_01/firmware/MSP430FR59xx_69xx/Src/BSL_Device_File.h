...
#ifndef __BSL_DEVICE_FILE_H
#define __BSL_DEVICE_FILE_H
#include <msp430.h>
...
// Configuration
// 
/*! Defines the default hardware entry sequence. 
 *  By default, the following methods are defined in BSL430_API_FRAM_CRYPTO.c
 ...
 */
    ...
    #define DEFAULT_STD_COMMANDS CONFIG_STDCMDS_DISABLED 
#else
    #define DEFAULT_STD_COMMANDS CONFIG_STDCMDS_ENABLED
#endif

/*! Define the behavior when incorrect password is detected (for standard RX_PWD)
 *      CONFIG_MASS_ERASE_PWD_DISABLED: Don't perform Mass erase 
 *      CONFIG_MASS_ERASE_PWD_ENABLED: Mass Erase the device
 */
 ...
 *  BSL Area = RX,  Rest of Mem = RW
 */
#define MPU_MPUSAM_DEFAULT      (0x3353)
/*! MPU access when writing memory including BSL
 *  BSL Area = RWX, Rest of Mem = RW
 */
#define MPU_MPUSAM_OPENBSL      (0x3373)
/*! MPU access when jumping to application:
 *  BSL Area = RX, User Mem = RWX, Info = RW 
 */
#define MPU_MPUSAM_APP          (0x3757)
   

//
//Device Specific Definitions
//
/*! Address where BSL Interface type is stored (in TLV for FR5969) */
#define BSL_STATE_ADDR          0x1A42
/*! Address where BSL Interface Configuration is stored (in TLV for FR5969) */
#define BSL_CONFIG_ADDR         0x1A43

#define MASS_ERASE_DELAY        44000   // ~5.5ms
#define BSL_PASSWORD_START      0xFFE0  // Start address of BSL Password
...
//******************************************************************************
#if (I2C_USCI_BASE == EUSCI_B0_BASE)
    // I2C Pin configuration
    #define I2C_PORT_SEL0       P1SEL0              
    #define I2C_PORT_SEL1       P1SEL1              
    #define I2C_PORT_DIR        P1DIR1
    #define SDA                 BIT6                // SDA on P1.6 - UCB0SDA
    #define SCL                 BIT7                // SCL on P1.7 - UCB0SCL
#ifdef __MSP430FR5969__
...
#endif
    // I2C Registers
    #define I2C_UCZNCTL1        UCB0CTLW0
    #define I2C_UCZNCTL0        UCB0CTLW0
    #define I2C_UCZNI2COA       UCB0I2COA0
    #define I2C_UCZNIE          UCB0IE
    #define I2C_UCZIV           UCB0IV
    #define I2C_UCZRXBUF        UCB0RXBUF
    #define I2C_UCZTXBUF        UCB0TXBUF
#else
...
#if (UART_USCI_BASE == EUSCI_A0_BASE)
    // UART Pin configuration
    #define UART_PORT_SEL1      P2SEL1              
    #define UART_PORT_SEL0      P2SEL0              
    #define UART_PORT_DIR       P2DIR1
    #define UART_TX             BIT0                // TX on P2.0 - UCA0TXD
    #define UART_RX             BIT1                // RX on P2.1 - UCA0RXD
#ifdef __MSP430FR5969__
    #define UART_PORT_CONFIG()   {UART_PORT_SEL0 &= ~(UART_TX + UART_RX);\
                                 UART_PORT_SEL1 |= (UART_TX + UART_RX);}
#elif defined __MSP430FR6989__
    #define UART_PORT_CONFIG()   {UART_PORT_SEL0 |= (UART_TX + UART_RX);\
                                 UART_PORT_SEL1 &= ~(UART_TX + UART_RX);}
#else
...
#endif
    // UART Registers
    #define UART_UCZNCTLW0      UCA0CTLW0
    #define UART_UCZNBRW        UCA0BRW
    #define UART_UCZNMCTLW      UCA0MCTLW
    #define UART_UCZNMCTLW_H    UCA0MCTLW_H
...
/* Change the project configuration based on requirements:
   |----------------------------|-----------------------------------|
   | Project configuration      |      Commands                     |
   |----------------------------|-----------------------------------|
   | "FR5969_Crypto"            |     Crypto BSL Commands           |
   | "FR5969_StandardwCrypto"   |     Standard + Crypto commands    |
   |----------------------------|-----------------------------------|

    Optionally, define the commands in this file as follows:
     #define CRYPTO_COMMANDS          1   // 1 to enable, 0 to disable
     #define FULL_STANDARD_COMMANDS   1   // 1 to enable, 0 to disable
*/ 
