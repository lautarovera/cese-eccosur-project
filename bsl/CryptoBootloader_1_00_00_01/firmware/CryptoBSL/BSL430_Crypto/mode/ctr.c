...
*Function:  mode_ctr
*Description: 
*             Performs encryption using counter (CTR) mode on a plaintext message
*             of size len blocks and stores it in msg.
*
*Parameters:
*             uint8_t *msg    Pointer to the message state array
*             uint8_t *key    Pointer to the array containing the key
*             uint8_t *iv     Pointer to the initialization vector
*             uint16_t msg_len  Mesage length in bytes
*******************************************************************************/

void mode_ctr(uint8_t *msg, uint8_t *key, uint8_t *iv,  uint16_t msg_len)
{
    ...
    {
        memcpy(temp, iv, CIPHER_BLOCK_SIZE);
        cipher_encrypt(temp, key);

        if (msg_left > CIPHER_BLOCK_SIZE) 
        {
            block_xor(msg+i, temp, CIPHER_BLOCK_SIZE);
            msg_left -= CIPHER_BLOCK_SIZE;
        }    
        else 
        {
            block_xor(msg+i, temp, msg_left);
        }    
...
/*******************************************************************************
*Function:  mode_ctr_encrypt
*Description: 
*             Performs encryption using counter (CTR) mode on a plaintext message
*              of size len blocks and stores it in the message state.
*
*Parameters:
*             uint8_t *msg    Pointer to the message state array
*             uint8_t *key    Pointer to the array containing the key
*             uint8_t *iv     Pointer to the initialization vector
*             uint16_t len    Mesage length in bytes
*******************************************************************************/

void mode_ctr_encrypt(uint8_t *msg, uint8_t *key, uint8_t *iv,  uint16_t len)
{
    mode_ctr(msg, key, iv, len);
}
#endif /* #ifdef INCLUDE_MODE_ENC */


#ifdef INCLUDE_MODE_DEC /* Include the AES encryption routine */
/*******************************************************************************
*Function:  mode_ctr_decrypt
*Description: 
*             Performs decryption using counter (CTR) mode on a ciphertext message
*              of size len blocks and stores it in the message state.
*
*Parameters:
*             uint8_t *msg    Pointer to the message state array
...
*             uint16_t  len     Mesage length in bytes
*******************************************************************************/
void mode_ctr_decrypt(uint8_t *msg, uint8_t *key, uint8_t *iv,  uint16_t len)
{
    mode_ctr(msg, key, iv, len);
}
#endif /* #ifdef INCLUDE_MODE_DEC */
