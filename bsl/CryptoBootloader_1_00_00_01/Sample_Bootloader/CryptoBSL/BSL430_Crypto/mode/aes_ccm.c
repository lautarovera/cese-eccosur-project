...
#ifdef INCLUDE_MODE_DEC /* Include the AES encryption routine */
/*******************************************************************************
*Function:  mode_ccm_decrypt
*Description:
*             Performs decryption and integrity+authenticity checks using 
*             AES-CCM mode on a ciphertext message of size len blocks and 
*             stores the plaintext in the message state.
*             The message length shall not exceed 2^16 bytes for this version.
*             Up to 14 octets of additional data may be used in this version.
*
*Parameters:
*             uint8_t *msg    Pointer to the message state
*             uint8_t *key    Pointer to the encryption key
*             uint8_t *iv     Pointer to the initialization vector (nonce)
*             uint8_t *aad    Pointer to the additional auth data (optional)
*             uint8_t msg_len Message size in bytes
*******************************************************************************/
#if CCM_AAD_LEN > 0
uint8_t mode_ccm_decrypt(uint8_t *msg, uint8_t *key, uint8_t *iv,  uint8_t *aad,
                         uint16_t msg_len)
#else
uint8_t mode_ccm_decrypt(uint8_t *msg, uint8_t *key, uint8_t *iv,  
                         uint16_t msg_len)
#endif
{
  ...
  uint8_t q;  
  uint8_t *tag_n;  
  uint8_t b0[CIPHER_BLOCK_SIZE];
  uint8_t temp[CIPHER_BLOCK_SIZE];
  uint8_t tag[CIPHER_BLOCK_SIZE];  
  
#if CCM_AAD_LEN > 0
  uint8_t adata[CIPHER_BLOCK_SIZE];
#endif
  ...
  q = 2;
#endif  
  memset(b0, 0x00, CIPHER_BLOCK_SIZE);
  b0[0] = ((CCM_AAD_LEN > 0)?0x40:0) | ((CCM_TAG_LEN - 2) >> 1) << 3 | (q-1);
  memcpy(&b0[1], iv+1, 15-q);  
#if   CCM_CTR_LEN == 2
  b0[14] = msg_len >> 8;
  b0[15] = msg_len & 0x00FF;
#endif
  ...
  iv[0] = 1;
#endif  
  memset(iv+15-(q-1), 0x00, q);
  memset(tag, 0x00, CIPHER_BLOCK_SIZE);
  memcpy(tag, (msg + msg_len), CCM_TAG_LEN);
  mode_ctr_decrypt(tag, key, iv, CCM_TAG_LEN);
  mode_ctr_decrypt(msg, key, iv, msg_len);
 
  /* compute the MAC */
  tag_n = &b0[0];
  mac_cbc_init(tag_n, key);
  
#if CCM_AAD_LEN > 0
  memset(adata, 0x00, CIPHER_BLOCK_SIZE);
  adata[0] = 0x00;  
  adata[1] = CCM_AAD_LEN;
  
  for (uint8_t i = 0; i < CCM_AAD_LEN; i++) 
  {
    adata[i+2] = aad[i];	
  }
  
  mac_cbc_next(tag_n, adata, key);
#endif
  
  for (uint16_t i = 0; i < msg_len; i+= CIPHER_BLOCK_SIZE)
  {    
    if (msg_len-i >= CIPHER_BLOCK_SIZE) 
    {
      mac_cbc_next(tag_n, msg+i, key);      
    }	
    else 
    {
      memset(temp, 0x00, CIPHER_BLOCK_SIZE);
      memcpy(temp, msg+i, msg_len-i);
      mac_cbc_next(tag_n, temp, key);
    }    
  }
  
  /* check if MAC is valid */
  if (memcmp_tsafe(tag, tag_n, CCM_TAG_LEN) != 0) 
  {

#ifdef CCM_CLEAR_ON_ERROR
