...
// Definitions of BSL lock condition
#define LOCKED          0x00        // BSL is locked  
#define UNLOCKED        0xA5A5      // BSL is unlocked 

// Definition of BSL configuration
#define CONFIG_MASS_ERASE_PWD_ENABLED   (0x00)  // Mass erase on incorrect password enabled
#define CONFIG_MASS_ERASE_PWD_DISABLED  (0xA5)  // Mass erase on incorrect password disabled
#define CONFIG_STDCMDS_DISABLED         (0x00)  // Standard commands are disabled
#define CONFIG_STDCMDS_ENABLED          (0x5A)  // Standard commands are enabled
#define CONFIG_ENTRY_STANDARD           (0x00)  // Using standard (TEST+RST) entry sequence
#define CONFIG_ENTRY_GPIO               (0x55)  // Use entry sequence using GPIO on reset

// Flags
#define PERFORM_MASS_ERASE              (0xAA)  // Indicates that a mass erase was initiated
#define APPLICATION_EXECUTED            (0x0A)  // Indicates an application was executed
#define UPDATE_STARTED                  (0xA0)  // Indicates a new update has started
...
// Version definitions
#define FRAM_MEMORY             0x30        // Using FRAM memory
#define FRAM_MEMORY_WCRYPTO     0x50        // Using FRAM with Crypto and MPU support
...
*******************************************************************************/
int8_t BSL430_toggleInfoLock(void);
...
*             uint16_t length  The amount of bytes to read
...
* *Parameters:
*           uint32_t addr   The address to which the function call should go
* *Returns:
...
*           uint16_t size       The number of bytes to write
*           uint8_t* data       The array of bytes to write (must be even aligned)
* *Returns:
*           SUCCESSFUL_OPERATION           Bytes written successfully
*           MEMORY_WRITE_CHECK_FAILED      A byte in data location post-write does 
*                                           not match data parameter
*                                          Note: write stops immediatly after a 
*                                           byte check fails
*           BSL_LOCKED                     Correct Password has not yet been given
*           VOLTAGE_CHANGE_DURING_PROGRAM  Voltage changed during write (of a single 
*                                           byte/word)
*******************************************************************************/
...
*           MEMORY_WRITE_CHECK_FAILED      Byte in data location post-write does 
*                                           not match data parameter
...
* *Parameters:
*           int8_t block_erase  Currently unused 1: erase block 0: erase segment
*           uint32_t addr       An address which is within the segment to be erased
* *Returns:
*           SUCCESSFUL_OPERATION           Segment erased
*           BSL_LOCKED                     Correct Password has not yet been given
*******************************************************************************/
int8_t BSL430_eraseLocation(int8_t block_erase, uint32_t addr);
