...
*         Initialization Vector (IV) is fixed to all zeros.
*         This version is meant to be used with CCM only. If used alone for
*         authentication, please complete the following TODO list
*
...
*           Prepend the message length to the first block of the message 
*           before MACing it.
*         - Use an Authentication Key (not an Encryption Key) to MAC
*           if used alone or in a construction that is not CCM
*******************************************************************************/
...
/*******************************************************************************
*Function:  mac_cbc_init
*Description: 
*             Calculates the first value making use of the IV fixed to zero,
*             the output is stored within msg_blk
*
*Parameters:
*             uint8_t *msg_blk   Pointer to the message block state array
*             uint8_t *key_auth  Pointer to the authorization-key array
*******************************************************************************/
void mac_cbc_init(uint8_t *msg_blk, uint8_t *key_auth)
{
    //uint8_t iv[CIPHER_BLOCK_SIZE] = {0};

    //block_xor(msg_blk, iv); // Not needed, we xor with 0s. Uncomment for other IVs.
    cipher_encrypt(msg_blk, key_auth);
}


/*******************************************************************************
*Function:  mac_cbc_next
*Description: 
*             Calculates the first value making use of the IV.
*             Initialization Vector (IV) is fixed to zero.
*
*Parameters:
*             uint8_t *chain     Pointer to the array used for chaining
*             uint8_t *msg_blk   Pointer to the message block state array
*             uint8_t *key_auth  Pointer to the authorization-key array
*******************************************************************************/
void mac_cbc_next(uint8_t *chain, uint8_t *msg_blk, uint8_t *key_auth)
{
    block_xor(chain, msg_blk, CIPHER_BLOCK_SIZE);
    cipher_encrypt(chain, key_auth);  
}
...
*Function:  mac_cbc
*Description: 
*             Creates a message authentication code and stores it in 
*             the address allocated for the tag.
*             Initialization Vector (IV) is fixed to zero.
*
*Parameters:
*             uint8_t *tag       Pointer to the tag array
*             uint8_t *msg       Pointer to the message state array
*             uint8_t *key_auth  Pointer to the authorization-key array
*             uint16_t msg_len   Message length in bytes
*******************************************************************************/
void mac_cbc(uint8_t *tag, uint8_t *msg, uint8_t *key_auth, uint16_t msg_len)
{
    memcpy(tag, msg, CIPHER_BLOCK_SIZE);
    mac_cbc_init(tag, key_auth);

    for (uint16_t i = CIPHER_BLOCK_SIZE; i < msg_len; i+= CIPHER_BLOCK_SIZE)
    {
        msg = (uint8_t*)msg + CIPHER_BLOCK_SIZE;
        mac_cbc_next(tag, msg, key_auth);    
    }
}
