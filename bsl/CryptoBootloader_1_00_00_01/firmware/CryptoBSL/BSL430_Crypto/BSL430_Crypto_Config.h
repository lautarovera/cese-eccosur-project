 ...
 *
 * --/COPYRIGHT--*/
#ifndef BSL430_CRYPTO_CONFIG_H_
#define BSL430_CRYPTO_CONFIG_H_

/*** User configuration *******************************************************/

/* 
 * Block and Key sizes for the crypto primitives
 * Values are in bytes 
 */
 
/* Encryption - Posible values for AES: Key = 16, 24, 32 bytes; Block = 16 */
#define CIPHER_KEY_SIZE 16
#define CIPHER_BLOCK_SIZE 16

/* Authentication - Posible values for AES: Key = 16, 24, 32 bytes; Block = 16 */
#define MAC_KEY_SIZE 16
#define MAC_BLOCK_SIZE 16

/* Key-encryption - Posible values for AES: Key = 16, 24, 32 bytes */
#define KEK_KEY_SIZE 16

/* AES-CCM Tag length. 16 bytes for this version */
#define CCM_TAG_LEN 16
/* AES-CCM Authentication data. Zero for this version */
#define CCM_AAD_LEN 0
/* AES-CCM CTR length in bytes. Only 2 is supported for this version */
#define CCM_CTR_LEN 2


/* 
 * Include encryption/decryption functions
 */
#define INCLUDE_CIPHER_ENC
//#define INCLUDE_CIPHER_DEC
//#define INCLUDE_MODE_ENC
#define INCLUDE_MODE_DEC

/* 
 * Algorith to use
 */
#define USE_CIPHER_AES_HW
//#define USE_CIPHER_AES_SW
...
/*** Definitions and checks - should be left unmodified************************/

#if !((CIPHER_KEY_SIZE == MAC_KEY_SIZE) && (MAC_KEY_SIZE == KEK_KEY_SIZE))
  #error "For this version, the sizes for the cipher, mac and kek keys must be the same."
#endif

#if !(CIPHER_BLOCK_SIZE == MAC_BLOCK_SIZE)
  #error "For this version, the sizes for the cipher and mac blocks must be the same."
#endif

#ifdef  USE_MODE_CTR
  #define MODE_H "ctr.h"
  #ifndef INCLUDE_CIPHER_ENC
  #define INCLUDE_CIPHER_ENC
  #endif
  #ifdef  INCLUDE_CIPHER_DEC
  #warning "INCLUDE_CIPHER_DEC is defined when using CTR mode, please make sure this inclusion was intended."
  //#undef  INCLUDE_CIPHER_DEC
  #endif
#endif /* USE_MODE_CTR */
   
#ifdef  USE_MODE_CCM
  #if   !defined(USE_CIPHER_AES_HW) && !defined(USE_CIPHER_AES_SW)
  #error "CCM must be used with an approved 128-bit block cipher."
  #endif
  #define MODE_H "aes_ccm.h"
  #ifndef INCLUDE_CIPHER_ENC
  #define INCLUDE_CIPHER_ENC
  #endif
  #ifdef  INCLUDE_CIPHER_DEC
  #warning "INCLUDE_CIPHER_DEC is defined when using CCM mode, please make sure this inclusion was intended."
  //#undef  INCLUDE_CIPHER_DEC
  #endif
  #ifdef INCLUDE_MODE_ENC /* Include the AES encryption routine */
  #error "Encryption is not supported for CCM in this version"
  #endif
  #if (CCM_CTR_LEN != 2)
  #error "This version only supports a CCM CTR length of two octets"
  #endif
  #if (CCM_TAG_LEN < 4) || (CCM_TAG_LEN > 16) || (CCM_TAG_LEN & 1)
  #error "Supported MAC lengths: 4, 6, 8, 10, 12, 14, 16"
  #endif
  #if CCM_AAD_LEN > 14
  #error "Only up to 14 bytes of AAD are supported for this version"
  #endif
  #if CCM_TAG_LEN != CIPHER_BLOCK_SIZE
  #warning "CCM MAC Tag size is not equal to the AES block size, please make sure this was intended."
  #endif
#endif /* USE_MODE_CTR */

#ifdef USE_MAC_CBC
  #define MAC_H "cbc_mac.h"
  #define INCLUDE_CIPHER_ENC
#endif /* USE_MAC_CBC */

#if !defined(INCLUDE_CIPHER_ENC) && !defined(INCLUDE_CIPHER_DEC)
#error "Encryption or decryption must be defined"
#endif

#if  defined(USE_CIPHER_AES_HW)
  #define CIPHER_H "aes.h"
  #ifdef  INCLUDE_CIPHER_ENC
  #define cipher_encrypt(state, key) aes_encrypt(state, key)
  #endif
  #ifdef  INCLUDE_CIPHER_DEC
  #define cipher_decrypt(state, key) aes_decrypt(state, key)
  #endif
  #if CIPHER_BLOCK_SIZE != 16
  #error "AES block-size must be 16 bytes"
  #endif
  #if (CIPHER_KEY_SIZE != 16) && (CIPHER_KEY_SIZE != 24) && (CIPHER_KEY_SIZE != 32)
  #error "AES key-size must be either 16, 24 or 32 bytes"
  #endif
#elif defined(USE_CIPHER_AES_SW)
  #define CIPHER_H "aes.h"
  #ifdef  INCLUDE_CIPHER_ENC
  #define cipher_encrypt(state, key) aes_encrypt(state, key)
  #endif
  ...
  #error "AES key-size for the software implementation should be 16 bytes"
  #endif
#endif /* USE_CIPHER_AES_HW */
   
#endif /* BSL430_CRYPTO_CONFIG_H_ */
