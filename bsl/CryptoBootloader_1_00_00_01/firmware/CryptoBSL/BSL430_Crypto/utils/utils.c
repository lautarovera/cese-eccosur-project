...
* Name:   utilities.c
* Description: 
*         Functions used by more than one cryptographic primitive
*
...
/*******************************************************************************
*Function:  memcmp_tsafe
*Description: 
*             Compares two arrays in a timing-independent manner.
*
*Parameters:
*             uint8_t *a      Pointer to the first array
*             uint8_t *b      Pointer to the second array
*             uint8_t len     Length in bytes to perform the comparison
*
*Returns:   0                     Arrays match   
*             any other value       Arrays do not match
*******************************************************************************/
uint8_t memcmp_tsafe(uint8_t *a, uint8_t *b, uint8_t len)
{
    uint8_t res = 0;
...
*Function:  block_xor
*Description: 
*             Performs an exclusive-or operation on two arrays and stores the
*             result in the space defined by the first parameter.
*
*Parameters:
*             uint8_t *a      Pointer to the first array
*             uint8_t *b      Pointer to the second array
*             uint8_t len     Length in bytes to perform the XOR
*******************************************************************************/
void block_xor(uint8_t *a, uint8_t *b, uint8_t len)
{
    uint8_t i;
...
*Function:  inc_iv
*Description: 
*             Increments by one the IV, stores the output in the same location.
*             NOTES: 
*             1. This function does not take care of overflows into the 
*                nounce part of the IV when incrementing the counter.
*             2. The counter is assumed to be at most *2 Bytes* long, if used 
*                in another application, change this accordingly!
*
*Parameters:
*             uint8_t *iv     Pointer to the array containing IV
*******************************************************************************/
void inc_iv(uint8_t *iv)
{  
    uint16_t temp;          /* short == 2 bytes (64 bits) */

    temp  = iv[CIPHER_BLOCK_SIZE-2] << 8;
    temp |= iv[CIPHER_BLOCK_SIZE-1];
    temp += 1;
    iv[CIPHER_BLOCK_SIZE-1] = temp & 0xFF;
    iv[CIPHER_BLOCK_SIZE-2] = temp >> 8;
}
