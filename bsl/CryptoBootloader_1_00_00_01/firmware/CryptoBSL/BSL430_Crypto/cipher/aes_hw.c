...
* Name:   aes_ccm.c
* Description: 
*         AES using the hardware accelerator
*
...
#ifdef USE_CIPHER_AES_HW
#include "aes.h"

#ifdef INCLUDE_CIPHER_ENC /* Include the AES encryption routine */
/*******************************************************************************
*Function:  aes_encr
*Description: 
*             Writes the plain-text state into the AES module
*             reads out the cipher-text and stores it in state
*
*Parameters:
*             uint8_t* state  Pointer to an array containing the AES State
*******************************************************************************/
static void aes_encr_hw(uint8_t *state)
{
  uint8_t i;
  /* Write the plaintext */
...
#endif /* #ifdef INCLUDE_CIPHER_ENC */


#ifdef INCLUDE_CIPHER_DEC /* Include the AES decryption routine */
/*******************************************************************************
*Function:  aes_decr
*Description: 
*             Writes the cipher-text state into the AES module
*             reads out the plain-text and stores it in state
*
*Parameters:
*             uint8_t* state  Pointer to an array containing the AES State
*******************************************************************************/
static void aes_decr_hw(uint8_t *state) 
{
  uint8_t i;

  /* Write the ciphertext into the AES module */
  for (i = 0; i < CIPHER_BLOCK_SIZE; i++) 
  {
    AESADIN_L = state[i];
  }
  ...
  /* Read out the plaintext */
  for (i = 0; i < CIPHER_BLOCK_SIZE; i++) 
  {
    state[i] = AESADOUT_L;
  }
}
#endif /* INCLUDE_CIPHER_DEC */

/*******************************************************************************
*Function:  aes_expand_key
*Description: 
*             Writes the key into the AES module for it to perform key expansion
*             this function is used for either encryption or decryption
*
*Parameters:
*             uint8_t* key  Pointer to an array containing the AES Key
*******************************************************************************/
static void aes_expand_key_hw(uint8_t *key)
{
  uint8_t i;

  /* Load the key into the AES module */
...
*Description: 
*             Performs encryption using the hardware AES module in MSP430
*             the output overwrites the input state
*
*Parameters:
*             uint8_t* key    Pointer to an array containing the AES Key
*             uint8_t* state  Pointer to an array containing the AES State
...
void aes_encrypt(uint8_t *state, uint8_t *key)
{
  ...
  /* Configuration - AES Operation: Encrypt; Key length: 128 */
  AESACTL0 = AESOP_0 + AESKL__128;
#elif CIPHER_KEY_SIZE == 24
  /* Configuration - AES Operation: Encrypt; Key length: 192 */
  AESACTL0 = AESOP_0 + AESKL__192;
#elif CIPHER_KEY_SIZE == 32
  /* Configuration - AES Operation: Encrypt; Key length: 256 */
  AESACTL0 = AESOP_0 + AESKL__256;
#else
#error "AES key-size must be either 16, 24 or 32 bytes"
#endif
  
  aes_expand_key_hw(key);
  aes_encr_hw(state);
  
}
#endif /* INCLUDE_CIPHER_ENC */
...
*Description: 
*             Performs decryption using the hardware AES module in MSP430
*             the output overwrites the input state
*
*Parameters:
*             uint8_t* key    Pointer to an array containing the AES Key
...
{
#if CIPHER_KEY_SIZE == 16
  /* Configuration - AES Operation: Decrypt; Key length: 128 */
  AESACTL0 = AESOP_1 + AESKL__128;
#elif CIPHER_KEY_SIZE == 24
  /* Configuration - AES Operation: Encrypt; Key length: 128 */
  AESACTL0 = AESOP_1 + AESKL__192;
#elif CIPHER_KEY_SIZE == 32
  /* Configuration - AES Operation: Encrypt; Key length: 128 */
  AESACTL0 = AESOP_1 + AESKL__256;
#else
#error "AES key-size must be 16, 24 or 32 bytes"
#endif
