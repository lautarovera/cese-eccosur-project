 ...
 * --/COPYRIGHT--*/
/*******************************************************************************
* Name:   BSL430_CRYPTO.c
* Description: 
*         BSL Cryptographic functions
*
...
/* IMPORTANT: Initial keys must be changed for each application */
__persistent EBSL_KEYS BSL430_keyEnc  = {.key={0}, .version=0}; // Encryption
__persistent EBSL_KEYS BSL430_keyEK   = {.key={0}, .version=0}; // Key-encryption
//__persistent EBSL_KEYS BSL430_keyAuth = {.key={0}, .version=0}; // Authentication

/* Metadata */
__persistent uint8_t fw_version = 0;
uint16_t last_packet_n;   // not persistent
uint16_t total_packets;   // not persistent
__persistent volatile uint8_t BSL_new_update_started = 0;

/* BSL area limits, should match the .xcl linker file */
extern uint16_t BSL_Limit_L, BSL_Limit_H, BSL_SignVectors;
#define BSL_LIMIT_L     ((uint16_t ) &BSL_Limit_L)
#define BSL_LIMIT_H     ((uint16_t ) &BSL_Limit_H)
#define BSL_SIGNVECTORS ((uint16_t ) &BSL_SignVectors)


/*******************************************************************************
*Function:    BSL430_Crypto_sec_init
*Description: function used to set the security checks when starting the BSL
*Returns:
*             Always 0
*******************************************************************************/
int8_t BSL430_Crypto_sec_init(void)
{  
    last_packet_n = 0;
    total_packets = 0;
    return 0;
...
/*******************************************************************************
*Function:    BSL430_Crypto_addrCheck
*Description: Verifies if data is allowed to be written into the memory location
*Parameters:
*             uint32_t addr         The address to which the byte should be written
*Returns:
*             BSL_MEMORY_AVAILABLE      Memory is available for R/W access
*             BSL_MEMORY_MPUPROTECTED   Memory is available but protected by MPU
*             BSL_MEMORY_IPEPROTECTED   Memory is unavailable 
*******************************************************************************/
int8_t BSL430_Crypto_addrCheck(uint32_t addr)
{
    int8_t exceptions = BSL_MEMORY_AVAILABLE;// Area available by default
     
    if ((addr >= BSL_LIMIT_L) && (addr <= BSL_LIMIT_H)) // CryptoBSL area (range defined in linker file)
    {
        if (addr >= BSL_SIGNVECTORS)
        {
            // IVT area can be written but it's protected by MPU
            exceptions = BSL_MEMORY_MPUPROTECTED;          
        }
        else
        {
            // BSL area can't be r/w
            exceptions = BSL_MEMORY_IPEPROTECTED;          
        }
    }
...
/*******************************************************************************
*Function:    BSL430_Crypto_dataChecks
*Description: Decrypts and/or verifies authenticity of the data sent,
*             on decryption, the plain-text will be stored in the data buffer
*Parameters:
*             uint8_t* data         Pointer to the array containing received data
*             uint16_t dataLength   Length of the data received
*Returns:
*             SUCCESSFUL_OPERATION  Data decrypted and/or verified
*             BSL_CRYPTO_ERROR      Error during decryption or verification
*******************************************************************************/
int8_t BSL430_Crypto_dataChecks(uint8_t* data, uint16_t dataLength)
{
#ifdef USE_MODE_CCM
    uint8_t *key;
    uint8_t *msg;
    ...
    key = (uint8_t*) &BSL430_keyEnc.key; // Encryption key
    iv  = (uint8_t*) data;     // First 16 bytes are the IV
    msg = (uint8_t*) data+16;  // Cipher-text starts after the 16 bytes IV
    msg_len = dataLength-16-CCM_TAG_LEN; // Remove the IV and Tag lengths

    if (mode_ccm_decrypt(msg, key, iv, msg_len) == TAG_INVALID)
    {
...
#else
#error "This version only supports AES in CCM mode"
#endif
  
}

/*******************************************************************************
*Function:    BSL430_Crypto_checkUpdateComplete
*Description: Checks if the total number of packets was sent.
*             If so, clears the "new_update_started" flag.
*Parameters:
*             uint8_t new_fw_ver    new firmware version
*******************************************************************************/
void BSL430_Crypto_checkUpdateComplete(uint8_t new_fw_ver)
{
    BSL430_openBSLMemory(); // Open BSL memory to write persistent variables
    if (last_packet_n == 1) 
    {
        BSL_new_update_started = UPDATE_STARTED;
    }
  
    if (last_packet_n == total_packets)
    {
        BSL_new_update_started = 0;
        last_packet_n = 0;
        total_packets = 0;
...
/*******************************************************************************
*Function:    BSL430_checkMetaData
*Description: Perform additional checks on the meta-data
*Parameters:
*             uint8_t* data         Pointer to the array with plain-text data
*Returns:
*             SUCCESSFUL_OPERATION  Data decrypted and/or verified
*             BSL_CRYPTO_ERROR      Error during decryption or verification
*******************************************************************************/
int8_t BSL430_Crypto_checkMetaData(uint8_t* data)
{
  ...
  new_fw_ver= (uint8_t)   data[16];
  packet_n  = (uint16_t) (data[17] << 8) | data[18];
  n_packets = (uint16_t) (data[19] << 8) | data[20];
  
  if (new_fw_ver < fw_version)
  {
    exceptions = BSL_CRYPTO_ERROR;
  }
  else
  {
    if (packet_n != last_packet_n+1)
    {
      exceptions = BSL_CRYPTO_ERROR;
    }
    else
    {
      last_packet_n = packet_n;
...
/*******************************************************************************
*Function:    BSL430_keyDecrypt
*Description: Decrypts and verifies authenticity of the key sent
*             *This function must always use an AE mode*
*Parameters:
*             uint8_t* data         Pointer to the array containing received data
*             uint16_t dataLength   Length of the data received
*Returns:
*             SUCCESSFUL_OPERATION  Decryption and verification were successful
*             BSL_CRYPTO_ERROR      Error during decryption or verification
*******************************************************************************/
int8_t BSL430_Crypto_keyDecrypt(uint8_t* data, uint16_t dataLength)
{
  ...
  key = (uint8_t*) &BSL430_keyEK.key; // Encryption key
  iv  = (uint8_t*) data;     // First 16 bytes are the IV
  msg = (uint8_t*) data+16;  // Cipher-text + tag come after the 16 bytes IV
  msg_len = dataLength-16-CCM_TAG_LEN; // Remove the IV and Tag lengths
...
#else
#error "Key decryption must always use an AE mode!"
#endif
  
}

/*******************************************************************************
*Function:    BSL430_keyUpdate
*Description: Updates the key material
*Parameters:
*             uint8_t* data         Pointer to the array containing received data
*             uint16_t dataLength   Length of the data received
*Returns:
*             SUCCESSFUL_OPERATION  Key was successfully updated
*             BSL_CRYPTO_ERROR      Error updating the key material
*******************************************************************************/
int8_t BSL430_Crypto_keyUpdate(uint8_t* data, uint16_t dataLength)
{
  uint8_t *new_key;
  uint8_t new_key_len;
...
#ifdef USE_MODE_CCM
  new_key = (uint8_t*) data+2;
  new_key_len = (uint8_t) dataLength;
  new_key_type = data[0];
  new_key_ver = data[1];
  
  if (new_key_len != CIPHER_KEY_SIZE)
  {
    return BSL_CRYPTO_ERROR;
  }
  else
  {
    switch (new_key_type)
    {
    case 0x00:	// data-encryption key
      if (new_key_ver < BSL430_keyEnc.version)      
      {
        return BSL_CRYPTO_ERROR;
      }

      for (int8_t i = 0; i < new_key_len; i++)
      {
        BSL430_keyEnc.key[i] = new_key[i];
      }

      BSL430_keyEnc.version = new_key_ver;
      break;
      
    case 0x01:	// authentication key
//      if (new_key_ver < BSL430_keyAuth.version)
//      {
        return BSL_CRYPTO_ERROR;
//      }
//      for (int8_t i = 0; i < new_key_len; i++)
//      {
//        BSL430_keyAuth.key[i] = new_key[i];
//      }
//      BSL430_keyAuth.version = new_key_ver;
      break;
      
    case 0x02:	// key-encryption key
      if (new_key_ver < BSL430_keyEK.version)
      {
        return BSL_CRYPTO_ERROR;
      }
      for (int8_t i = 0; i < new_key_len; i++)
      {
        BSL430_keyEK.key[i] = new_key[i];
      }
      BSL430_keyEK.version = new_key_ver;
      break;
